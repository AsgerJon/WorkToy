"""
TestAbstractObject tests the AbstractObject class central to the 'worktoy'
library, found in the 'worktoy.static' module.
"""
#  AGPL-3.0 license
#  Copyright (c) 2025 Asger Jon Vistisen
from __future__ import annotations

from unittest import TestCase
from typing import TYPE_CHECKING

from worktoy.static import AbstractObject
from worktoy.static.zeroton import THIS, OWNER
from worktoy.text import stringList
from worktoy.waitaminute import MissingVariable, TypeException
from worktoy.waitaminute import ReadOnlyError, ProtectedError
from worktoy.waitaminute import _Attribute  # NOQA

if TYPE_CHECKING:  # pragma: no cover
  from typing import TypeAlias, Any

  Bases: TypeAlias = tuple[type, ...]


class TestAbstractObject(TestCase):
  """
  TestAbstractObject tests the AbstractObject class central to the 'worktoy'
  library, found in the 'worktoy.static' module.
  """

  @classmethod
  def tearDownClass(cls) -> None:
    import sys
    import gc
    sys.modules.pop(__name__, None)
    gc.collect()

  def test_free(self) -> None:
    """
    Tests that an 'AbstractObject' instance not created inside a class
    body still behaves as expected.
    """
    obj = AbstractObject.__new__(AbstractObject)
    self.assertIsNone(getattr(obj, '__pos_args__'))
    self.assertIsNone(getattr(obj, '__key_args__'))
    posArgs = obj._getPositionalArgs()
    keyArgs = obj._getKeywordArgs()
    self.assertIsInstance(posArgs, tuple)
    self.assertIsInstance(keyArgs, dict)
    self.assertFalse(posArgs and keyArgs)
    with self.assertRaises(MissingVariable) as context:
      _ = obj.getFieldName()
    e = context.exception
    self.assertEqual(e.varName, '__field_name__')
    self.assertEqual(e.varType, (str,))
    with self.assertRaises(MissingVariable) as context:
      _ = obj.getFieldOwner()
    e = context.exception
    self.assertEqual(e.varName, '__field_owner__')
    self.assertEqual(e.varType, (type,))

  def test_bad_arg_types(self) -> None:
    """
    Tests that 'AbstractObject' raises 'TypeException' when the
    '__pos_args__' and '__key_args__' attributes have the wrong types.
    """
    obj = AbstractObject.__new__(AbstractObject)
    setattr(obj, '__pos_args__', 69)
    setattr(obj, '__key_args__', 420)
    with self.assertRaises(TypeException) as context:
      _ = obj._getPositionalArgs()
    e = context.exception
    self.assertEqual(e.varName, '__pos_args__')
    self.assertEqual(e.actualObject, 69)
    self.assertEqual(e.actualType, int)
    self.assertEqual({*e.expectedType}, {list, tuple})
    with self.assertRaises(TypeException) as context:
      _ = obj._getKeywordArgs()
    e = context.exception
    self.assertEqual(e.varName, '__key_args__')
    self.assertEqual(e.actualObject, 420)
    self.assertEqual(e.actualType, int)
    self.assertEqual(e.expectedType, (dict,))

  def test_THIS(self, ) -> None:
    """
    Tests that the 'THIS' property returns the correct value.
    """

    class Bar(AbstractObject):
      """
      This subclass of 'AbstractObject' is actually being tested here.
      """

      def __instance_get__(self, **kwargs, ) -> Any:
        """
        This method is called when the 'THIS' property is accessed.
        It returns the 'Bar' instance itself.
        """
        keyArgs = self._getKeywordArgs()
        if keyArgs:
          return keyArgs['foo'].__inner_value__
        posArgs = self._getPositionalArgs()
        return posArgs[0].__inner_value__

    class Foo:
      """
      This class owns the 'AbstractObject' object being tested.
      """

      __inner_value__ = 69
      bar = Bar(foo=THIS, )
      baz = Bar(THIS, )

    foo = Foo()
    self.assertEqual(foo.bar, foo.baz)

  def test_str_repr(self) -> None:
    """
    Tests the string representation of 'AbstractObject'.
    """
    obj = AbstractObject()
    strObj = str(obj)
    reprObj = repr(obj)
    self.assertIn(AbstractObject.__module__, strObj)
    self.assertIn(AbstractObject.__name__, strObj)
    self.assertIn(AbstractObject.__module__, reprObj)
    self.assertIn(AbstractObject.__name__, reprObj)

  def test_accessors(self) -> None:
    """
    Tests the accessors of 'AbstractObject'.
    """

    class Prop(AbstractObject):
      """
      This subclass of 'AbstractObject' is used to test the accessors.
      """

      def __instance_get__(self, **kwargs) -> Any:
        """
        This method is called when the 'Prop' instance is accessed.
        It returns the value of the 'value' attribute.
        """
        pvtName = self.getPrivateName()
        try:
          out = getattr(self.instance, pvtName)
        except Exception as exception:
          raise MissingVariable(pvtName, object) from exception
        else:
          return out

    class Foo:
      """
      This class owns the 'Prop' object being tested.
      """

      bar = Prop()

    foo = Foo()
    privateName = Foo.bar.getPrivateName()
    with self.assertRaises(MissingVariable) as context:
      _ = foo.bar
    e = context.exception
    self.assertEqual(e.varName, privateName)
    self.assertEqual(e.varType, (object,))

    with self.assertRaises(ReadOnlyError) as context:
      foo.bar = 69
    e = context.exception
    self.assertIs(e.owningInstance, foo)
    self.assertIs(e.descriptorObject, Foo.bar)
    self.assertIsInstance(e.existingValue, MissingVariable)
    self.assertEqual(e.existingValue.varName, privateName)
    self.assertEqual(e.existingValue.varType, (object,))
    self.assertEqual(e.newValue, 69)

    with self.assertRaises(ProtectedError) as context:
      del foo.bar
    e = context.exception
    self.assertIs(e.owningInstance, foo)
    self.assertIs(e.descriptorObject, Foo.bar)
    self.assertIsInstance(e.existingValue, MissingVariable)
    self.assertEqual(e.existingValue.varName, privateName)
    self.assertEqual(e.existingValue.varType, (object,))

  def test_kwargs_parser(self) -> None:
    """
    Tests the keyword arguments parser of 'AbstractObject'.
    """
    obj = AbstractObject(breh=True, x=69, X='420 lmao')
    typeArgs = (complex, float, int)
    kwargs = obj._getKeywordArgs()
    intX, _ = obj.parseKwargs(420, *typeArgs, 'x', 'X', **kwargs)
    bre, _ = obj.parseKwargs(complex, float, 'breh', **kwargs)
    bre2, _ = obj.parseKwargs(complex, 'breh', **kwargs)
    bre3, _ = obj.parseKwargs(float, 'breh', **kwargs)
    bre4, _ = obj.parseKwargs(float, int, 'breh', **{})
    derp, _ = obj.parseKwargs(int, **kwargs)
    with self.assertRaises(TypeException) as context:
      __, _ = obj.parseKwargs(float, int, 'breh', **{'breh': 'lmao'})
    e = context.exception
    self.assertEqual(e.varName, 'breh')
    self.assertEqual(e.actualObject, 'lmao')
    self.assertEqual(e.actualType, str)
    self.assertEqual({*e.expectedType}, {float, int})
    self.assertEqual(intX, 69)

  def test_owner(self, ) -> None:
    """
    Tests that OWNER can be resolved correctly from keyword arguments.
    """

    class CLS(AbstractObject):
      """
      This subclass of 'AbstractObject' is used to test the owner resolution.
      """

      def __instance_get__(self, **kwargs) -> Any:
        """
        This method is called when the 'Prop' instance is accessed.
        It returns the value of the 'value' attribute.
        """
        keyArgs = self._getKeywordArgs()
        ownerKeys = stringList("""owner, cls, type, class_""")
        try:
          out, _ = self.parseKwargs(type, *ownerKeys, **keyArgs)
        except Exception as exception:
          raise exception
        else:
          return out

    class Foo:
      """
      This class owns the 'CLS' object being tested.
      """

      cls1 = CLS(owner=object, )
      cls2 = CLS(owner=OWNER)
      cls3 = CLS(class_=object, )

    foo = Foo()
    clsN = [foo.cls1, foo.cls3, ]
    for cls in clsN:
      self.assertIs(cls, object)
    self.assertIs(foo.cls2, Foo)
    self.assertEqual(str(Foo.cls1), repr(Foo.cls1))

  def test_missing_field_owner(self) -> None:
    """
    Tests that 'AbstractObject' raises 'MissingVariable' when the field owner
    is not set.
    """
    obj = AbstractObject.__new__(AbstractObject)
    with self.assertRaises(MissingVariable) as context:
      _ = obj.getFieldOwner()
    e = context.exception
    self.assertEqual(e.varName, '__field_owner__')
    self.assertEqual(e.varType, (type,))

  def test_present_field_owner(self) -> None:
    """
    Tests that 'AbstractObject' returns the field owner when it is set.
    """

    class Foo:
      bar = AbstractObject()

    self.assertIs(Foo.bar.getFieldOwner(), Foo)
