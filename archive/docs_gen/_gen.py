...
where they are defined, so the shown source is click-through: a worktoy
...
def _token_html(css: str, value: str, href: str) -> str:
  ...
  navigate away from the worktoy docs."""
  inner = html.escape(value, quote=False)
  if css:
...
def _add_alias_site(
  ...
  as it appears in the 'import' line is itself a link."""
  column = getattr(alias, "col_offset", None)
  lineno = getattr(alias, "lineno", None)
  if column is not None and lineno is not None:
    sites.append((starts[lineno - 1] + column, local))
...
def _definition_links(pyfile: Path) -> dict:
  ...
  defined. Four kinds are resolved, in order of precedence:
    ...
    - a name imported from a documented worktoy symbol, to that symbol's
      page at '#line-N';
  ...
  #  Imported names: worktoy symbols and standard-library members.
  bindings = {}
  alias_sites = []
  for node in ast.walk(tree):
